...
The skyline is the island as it really looks from a boat about 10 km east of Douglas, from Langness lighthouse on the left to Maughold Head on the right: Port St Mary, Douglas Head and the promenade, the hills rising to Snaefell, Laxey in its glen with the Great Laxey Wheel, Ramsey below the Albert Tower with the Queen's Pier, and Maughold Head's light on its cliff. The hills are drawn six times their real height so they read at this size. The towns, the wheel and the lighthouses stand at their true bearings from the boat but are drawn large enough to see. It is scenery. A column shows a moment in time, not a place, and the forecast is for a single point at Douglas.
...
| Lit windows in Port St Mary, Douglas, Laxey and Ramsey | Darkness at that hour |
...
The page opens on the zoom you last used in that browser, or 48 hours the first time. Query options: `?demo` shows a made-up week that exercises every kind of weather, `?hours=168`, `48`, `24` or `12` opens on that view regardless, and `?stretch=` sets how many times their real height the hills are drawn (6 unless you say otherwise).
...
- Skyline: `profile.js`, built once by `tools/build_profile.py` from the SRTM 1 arc-second tile N54W005 (AWS Terrain Tiles), about 30 m between points. The script stands on a boat at 54.150° N, 4.310° W with the eye 6 m above the sea, and for 720 compass bearings from south-west round to due north it finds the steepest angle up to the ground, allowing for the curve of the earth and for refraction. Ground within 11 km makes the front sheet of paper, ground within 16.5 km the middle one, and everything the back one. Lighthouses and landmarks are placed by their bearing from the same boat. The tile puts Snaefell at 622 m, North Barrule at 561 m and South Barrule at 477 m, against 621, 565 and 483 m. The page never calls an elevation service.
